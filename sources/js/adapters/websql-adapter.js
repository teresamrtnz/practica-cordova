//para borrar bbdd: Linux: ~/.config/google-chrome/Default/databases
//Más info: http://martinsikora.com/how-to-delete-web-sql-database-in-google-chrome

var WebSqlAdapter = function () {

    this.inicializar = function () {
        var deferred = $.Deferred();
        this.db = window.openDatabase("Futbolistas", "1.0", "Futbolistas", 200000);
    ...
    var crearTabla = function (tx) {
        tx.executeSql('DROP TABLE IF EXISTS futbolistas');
        var sql = "CREATE TABLE IF NOT EXISTS futbolistas ( " +
    ...
    var insertarDatos = function (tx, futbolistas) {
        ...
        {"id": 12, "nombre": "Carles", "apellido": "Puyol", "posicion": "Defensa", "equipo": "F.C.Barcelona", "dorsal": 5, "desc":"Toda una institución en el club. En sus 14 años defendiendo los colores del Barcelona, el 'gran' capitán lo ha ganado absolutamente todo. La pasada campaña sólo pudo disputar 15 partidos de Liga, pero su fortaleza, seguridad y jerarquía en el eje de la zaga hacen de él un jugador insustituible.", "imagen": "puyol.jpg"},
       ...
       {"id": 15, "nombre": "Victor", "apellido": "Valdés", "posicion": "Portero", "equipo": "F.C.Barcelona", "dorsal": 1, "desc":"El experimentado guardameta volverá a defender la portería azulgrana tras más de diez temporadas en la élite. Galardonado en cinco ocasiones con el Trofeo Zamora al portero menos goleado, destaca por sus enormes reflejos, manejo de los pies y liderazgo en el terreno de juego.", "imagen":"victorvaldes.jpg"},
        ...
        var sql = "INSERT OR REPLACE INTO futbolistas " +
            "(id, nombre, apellido, posicion, equipo, dorsal, desc, imagen) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        var e;
        for (var i = 0; i < l; i++) {
            e = futbolistas[i];
            tx.executeSql(sql, [e.id, e.nombre, e.apellido, e.posicion, e.equipo, e.dorsal, e.desc, e.imagen],
                function () {
